...
class Class(models.Model):
    ...
    def get_short_name(self):
        return "".join([x[0] if x[0].isupper() else "" for x in self.name.split(" ")])
...
class Member(models.Model):
    ...
    teaching_class = models.ForeignKey(Class, verbose_name="Class", on_delete=models.CASCADE)
...
class Lesson(models.Model):
    ...
    def get_short_title(self):
        return "".join([x[0] if x[0].isupper() else "" for x in self.title.split(" ")])
